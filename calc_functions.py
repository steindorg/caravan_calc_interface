...
def fob_verd(listaverd_eu):
    ...
    return round( listaverd_med_afsl + afreiknadur_19_prosent_vsk,0 )
...
def tollur_13(listaverd_eu, gengi, frakt):
    return round(cif_verd(listaverd_eu, gengi, frakt) * 0.13, 0)
    
def vsk(listaverd_eu, gengi, frakt):
    return round(( cif_verd(listaverd_eu, gengi, frakt) + tollur_13(listaverd_eu, gengi, frakt) ) * 0.24,0)
...
def til_landsins(listaverd_eu, gengi, frakt):
    ...
    return round(verd_til_landsins,0)